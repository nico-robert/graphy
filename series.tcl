...
oo::class create graphy::series::Bar {
    ...
    constructor {args} {
        ...
        graphy::setdef options -type            -validvalue {}  -type str       -default "bar"
...
proc graphy::barSeries {c chart series index indexSeries series_bar_len y_axis_values_list max_height xOpts} {
    ...
    set widthB         [graphy::dictGet    $optsSeries -backgroundStyle width]
    ...
    foreach value [graphy::dictGet $optsSeries -data] {
        if {$value in {null _}} {incr i ; continue}
        ...
        if {$label_show} {
            lappend series_labels [list \
                point [list [expr {$left + $half_bar_width}] $y] \
                text $value \
            ]
        }
        incr i
